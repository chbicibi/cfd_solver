...
module hsmac2d
    ...
    subroutine main
        ...
        do while (icycle < ncycle)
            ...
            ! 速度場の計算
            call calc_velociry(uo, vo, po, to, un, vn)
            ! 速度の境界条件の処理
            call bind_velocity(un, vn, fi)

            ! 圧力反復
            do itr = 1, nitr
                ! 圧力場の計算
                call calc_pressure(itr, un, vn, po, iflg)
    ...
    subroutine read_inputfile(filename)
        ...
        open(newunit=unit, file=filename)
            ...
            ! nitr   -> 圧力反復回数の上限
            ! ncycle -> 時間進行回数の上限
            read(unit, *) itype, icycle, nitr, ncycle
            ...
            ! pr -> プラントル数
            ! gr -> グラスホフ数
            read(unit, *) dt, re, pr, gr
            ! in2d.txt中のコメント行(23-24行目)のスキップ
            do i = 1, 2
                read(unit, *)
            end do
            read(unit, *) nx, ny, stridex, stridey
            ...
            ! irelp  -> 1:圧力を正規化する
            ! method -> 使用しない
            read(unit, *) dlx, dly, irelp, method, iter
            ! in2d.txt中のコメント行(29-30行目)のスキップ
            do i = 1, 2
                read(unit, *)
            end do
            read(unit, *) iout, dirname
        close(unit)
    ...
    ! *                    速度場の計算
    ! *********************************************************************
    subroutine calc_velociry(uin, vin, pin, tin, uout, vout)
        real(8), intent(in) :: uin(0:, 0:), vin(0:, 0:), pin(0:, 0:), tin(0:, 0:)
        ...
        subroutine calcv_vec(iy)
            ...
            ! 浮力項(buov)の計算
            tv   = (tin(1:nx, iy) + tin(1:nx, iy+1)) * 0.5d0
            buov = buo * tv
            ! 拡散項(difv)の計算
        ...
        subroutine calcv_mat
            ...
            ! 浮力項(buov)の計算
            tv   = (tin(1:nx, 1:ny-1) + tin(1:nx, 2:ny)) * 0.5d0
            buov = buo * tv
            ! 拡散項(difv)の計算
    ...
    ! *                        圧力場の計算
    ! *********************************************************************
    subroutine calc_pressure(itr, u, v, p, iflg, msg)
        integer, intent(in) :: itr
        real(8), intent(inout) :: u(0:, 0:), v(0:, 0:), p(0:, 0:)
        integer, intent(out) :: iflg
        character(120), intent(out), optional :: msg(1)
        ...
        ! epsp1 = epsp * (1 + 9d0 * (itr - 1) / (nitr - 1))
        ! epsp1 = epsp * max(10 ** ((itr - 100d0) / (nitr - 100d0)), 1.0d0)
        ! epsp1 = epsp * max(1 + 9d0 * ((itr - 100d0) / (nitr - 100d0)) ** 2, 1d0)
        epsp1 = epsp
        ...
        if (present(msg)) then
            write(msg, "('itr=',i6,' div_max(',2i5,')=',es12.5,' omg=',f6.3,' epsp=',es10.3)") &
                itr, pos_max, div_max, omg1, epsp1
        else if (mod(itr, 100) == 0) then
            ! 圧力計算の回数を100回ごとに表示
            print "('P:cycle=',i8,' itr=',i6,' div_max(',2i5,')=',es12.5,' omg=',f6.3,' epsp=',es10.3)", &
                icycle, itr, pos_max, div_max, omg1, epsp1
        end if
    ...
    ! *                    速度の境界条件の処理
    ! *********************************************************************
    subroutine bind_velocity(u, v, field)
        real(8), intent(inout) :: u(0:, 0:), v(0:, 0:), field(0:, 0:)
        integer :: ix, iy
        ...
        ! 物体内部の処理
        u(0:nx, 0:ny) = u(0:nx, 0:ny) * (field(0:nx, 0:ny) + field(1:nx+1, 0:ny  )) * 0.5d0
        v(0:nx, 0:ny) = v(0:nx, 0:ny) * (field(0:nx, 0:ny) + field(0:nx,   1:ny+1)) * 0.5d0
